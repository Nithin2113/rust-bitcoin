...
impl<E: fmt::Debug, I: Iterator<Item = Result<u8, E>>> IterReader<E, I> {
    ...
    fn decode<T: Decodable>(mut self) -> Result<T, DecodeError<E>> {
        ...
        match (result, self.error) {
            ...
            (Ok(_), Some(error)) => unreachable!("{} silently ate the error: {:?}", core::any::type_name::<T>(), error),
            ...
            (Err(consensus::encode::Error::Io(io_error)), de_error) => unreachable!("unexpected I/O error {:?} returned from {}::consensus_decode(), deserialization error: {:?}", io_error, core::any::type_name::<T>(), de_error),
            (Err(consensus_error), Some(de_error)) => unreachable!("{} should've returned `Other` I/O error because of deserialization error {:?} but it returned consensus error {:?} instead", core::any::type_name::<T>(), de_error, consensus_error),
        }
    }
}
